lopp:			// next address to next element
				// ret R0                R0
				// uint32_t *lopp(uint32_t *pu32){
				//	return pu32 + 1 ;  //  pointer arithmetic!
				//}

				// R0 = pu32 (a 32-bit ptr to uint32_t)
				ADD	R0,R0,#4	// R0 = pu16 + 1
				BX	LR
...
_start:			// Program Starts Here
				...
				LDR		R0,=wordArray	// get the address for our array
				BL		lopp
				LDR		R1,[R0]
